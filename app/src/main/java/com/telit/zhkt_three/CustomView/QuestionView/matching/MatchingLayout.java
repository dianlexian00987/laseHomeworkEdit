...
//                saveTrack = sharedPreferences.getString("my_line_answer", "");
    ...
    public void drawLigature(String leftId, String rightId, boolean isResult) {
        ...
        if (leftLayout != null && left != null) {
            sx = leftLayout.getLeft() + left.getLeft() + left.getWidth();
            sy = leftLayout.getTop() + left.getTop() + (left.getHeight() * 1.0f) / 2.0f;
        ...
        if (rightLayout != null && right != null) {
            ex = rightLayout.getLeft() + right.getLeft();
            ey = rightLayout.getTop() + right.getTop() + (right.getHeight() * 1.0f) / 2.0f;
